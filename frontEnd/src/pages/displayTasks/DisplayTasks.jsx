...
export default function DisplayTasks() {
	...
	function handleAdd() {
		addTask(desc);
		setDesc(""); // Clear the input field after adding task
	}
								...
								{tasks
									...
									.map((task, index) => (
										<div className="task" key={index}>
											...
											<p>id: {task.id}</p>
											<p>TASK: {task.desc}</p>
											<p>STATUS: {task.progress ? "completed" : "NOT completed"}</p>
											<button
												className="butt"
												onClick={() => handleProgress(task.id)}
											>
												{task.progress ? "UNDO" : "DONE"}
											</button>
											<button className="butt" onClick={() => deleteTask(task.id)}>
												DELETE
											</button>
										</div>
									))}
